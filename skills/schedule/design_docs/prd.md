...
The concrete file shape, field-by-field rules, and examples are defined canonically in **`data_model.md`**; engineering choices (modules, CLIs, algorithms) are in **`architecture.md`**. This document states **what the product must do**, from the customer's perspective. Sections are top-level headings (`N.`); every requirement is addressed by its `N.M` number.
...
**D.1.** **Live refresh.** While developing a schedule, the Gantt updates when the schedule file changes without manual refresh. Shelved; implementation plan and rationale in `live_refresh.md`.
