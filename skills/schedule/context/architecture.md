...
An amber vertical line labeled "Today" (`.today-line` / `.today-tag`, PRD **R31**) marks the current date. The line is drawn in its **own overlay SVG** (`renderTodayOverlay`, `.today-overlay`) at `edges[todayOffset]` — the same master-grid lookup as every bar, so it aligns with the date header and scrolls with the content. The overlay sits at a z-index above the rows (including a locked context band) but below the sticky header and gutter, so the line overlays content yet never crosses the date scale; the "Today" tag is a separate HTML element in the sticky gutter lane. `todayColumnOffset()` computes today from `new Date()` **at render time** (today is a property of *when the chart is viewed*, not a compute-time value, so it is never baked into `gantt_data.json`) and returns `null` when today falls outside `[range.start, range.end]`. Because `dateRange()` derives the axis only from item, people, and events dates, today never extends the range — an out-of-range today simply draws nothing, and a schedule opened long after it ended keeps its own width.
